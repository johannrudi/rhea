...
/**
 * Calculates the accuracy for solving the linearized system
                          ...
                          rhea_newton_options_t *opt)
{
  ...
  if (iter < opt->lin_rtol_init_n_iter) { /* if setting init rtol */
    lin_rtol = opt->lin_rtol_init;

    RHEA_GLOBAL_INFOF ("Newton iter %d -- %s: Use initial rtol %.3e\n",
                       iter, this_fn_name, lin_rtol);
  }
  else { /* otherwise setting adaptive rtol */
    const int           prog_reduction_active =
                          (0 < opt->adaptive_lin_rtol_progressive_n_iter);
    double              prog_reduction = 1.0;
    double              lin_rtol_max = opt->adaptive_lin_rtol_max;
    ...
    const double        res_norm_reduction_prev =
                          SC_MIN (residual->res_norm_reduction_prev, 1.0);
    const double        res_norm_reduction_curr =
                          SC_MIN (residual->res_norm_reduction_curr, 1.0);

    /* check input */
    RHEA_ASSERT (0.0 < residual->res_norm_reduction_prev);
    RHEA_ASSERT (0.0 < residual->res_norm_reduction_curr);
    ...
    if (prog_reduction_active) {
      ...
      const double        rtm = lin_rtol_max;

      prog_reduction = (exp (-iter2/prog_n_iter2) + eps/rtm) / (1.0 + eps/rtm);
    ...
    RHEA_GLOBAL_INFOF (
        "Newton iter %d -- %s: Candidate for adaptive rtol %.3e, "
        "max %.3e, min %.3e, min threshold %.3e, progressive reduction %.3e\n",
        iter, this_fn_name, lin_rtol, lin_rtol_max, lin_rtol_min,
    ...
    RHEA_GLOBAL_INFOF ("Newton iter %d -- %s: Set adaptive rtol %.3e\n",
                       iter, this_fn_name, lin_rtol);
  }
                              ...
                              rhea_newton_options_t *opt);
{
  ...
  RHEA_GLOBAL_INFOF ("Newton iter %d -- Into %s\n", iter, this_fn_name);
...
//rhea_stokes_problem_nonlinear_solve_linearized (
  nl_problem->solve_linearized (
      step->vec, residual->vec, lin_res_norm_rtol, lin_iter_max,
  ...
  //TODO
  //ymir_nlstokes_op_apply (step_vec, lin_residual_vec, nl_problem->stokes_op);
  ...
  RHEA_GLOBAL_INFOF ("Newton iter %d -- Done %s (num iter %i, "
                                              ...
                                              const int print_condition)
{
  ...
  if (print_condition) {
    RHEA_GLOBAL_INFOF (
        "Newton iter %d -- %s: Descend condition: residual norm <= %.3e\n",
                                ...
                                rhea_newton_options_t *opt)
{
  ...
  solution_prev = ymir_vec_copy (solution);
  ...
  //TODO
  //rhea_stokes_problem_nonlinear_update_operator(nl_problem, solution);
    nl_problem->update_operator (solution, nl_problem->data);

    /* compute residual for new solution */
  //TODO
  //rhea_problem_compute_residual (
  //    residual_vec, solution, nl_problem);
  //res_norm = rhea_problem_compute_norm (
  //    residual_vec);
    nl_problem->compute_residual (residual->vec, solution, nl_problem->data);
    res_norm = nl_problem->compute_norm (res_norm_comp, residual->vec,
                                         nl_problem->data);
    ...
    RHEA_GLOBAL_INFOF ("Newton iter %d -- %s: check # %i, step length %.3e, "
  ...
  if (search_success) {
    RHEA_GLOBAL_INFOF (
        "Newton iter %d -- %s: Line search successful, step length %.3e\n",
...
//      solver_options->nl_check_derivative);
    ...
    if (step_length < 0.0) {
      RHEA_GLOBAL_INFOF (
          "Newton iter %d -- %s: Line search failed, "
...
//                                    nl_solver_type);
  ...
  RHEA_GLOBAL_INFOF ("Newton iter %d -- Done %s\n", iter, this_fn_name);
}
                   ...
                   rhea_newton_options_t *opt)
{
   ...
   */
  {
    ...
    /* create step */
    step_vec = ymir_vec_template (solution);
    rhea_newton_step_init (&step, step_vec);

    /* create residual */
    res_norm_comp = RHEA_ALLOC (double, nl_problem->residual_norm_n_components);
  ...
  for (iter = iter_start; iter < iter_max; iter++) { /* BEGIN: Newton iter */
     ...
     */
    {
      ...
      if (residua.norm_reduction < res_norm_rtol) {
  ...
  if (iter_max == iter && res_norm_rtol <= residua.norm_reduction) {
    RHEA_GLOBAL_PRODUCTIONF (
