...
int
rhea_composition_exists (rhea_composition_options_t *opt)
{
  return (NULL != opt && RHEA_COMPOSITION_NONE != opt->type);
}
...
void
rhea_composition_add_rhs_vel (ymir_vec_t *rhs_vel,
                              ymir_vec_t *comp_density,
                              rhea_composition_options_t *opt)
{
  rhea_composition_rhs_vel_fn_data_t  data;
  ...
  case RHEA_COMPOSITION_DENSITY:
    ...
    ymir_cvec_set_function (rhs_vel, rhea_composition_add_rhs_vel_fn, &data);
    break;
  default: /* unknown composition type */
    RHEA_ABORT_NOT_REACHED ();
  }
