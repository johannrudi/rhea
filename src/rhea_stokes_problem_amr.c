...
typedef struct rhea_stokes_problem_amr_data
{
  ...
  sc_dmatrix_t       *composition_density_original;
  sc_dmatrix_t       *composition_density_adapted;

//sc_dmatrix_t       *composition_viscosity_original;
//sc_dmatrix_t       *composition_viscosity_adapted;
                                      ...
                                      rhea_stokes_problem_amr_data_t *amr_data)
{
  ...
  composition_density = rhea_stokes_problem_get_composition_density (
      stokes_problem);
  if (composition_density != NULL) {
    rhea_composition_destroy (composition_density);
  }
                                      ...
                                      rhea_stokes_problem_amr_data_t *amr_data)
{
  ...
  if (amr_data->composition_density_original != NULL) {
    sc_dmatrix_destroy (amr_data->composition_density_original);
  }
  if (amr_data->composition_density_adapted != NULL) {
    sc_dmatrix_destroy (amr_data->composition_density_adapted);
  }
  ...
  amr_data->composition_density_original = NULL;
  amr_data->composition_density_adapted = NULL;
}
...
rhea_stokes_problem_amr_data_initialize_fn (p4est_t *p4est, void *data)
{
  ...
  ymir_vec_t         *composition_density =
    rhea_stokes_problem_get_composition_density (d->stokes_problem);
  const int           has_temp = (temperature != NULL);
  const int           has_vel_press = (velocity_pressure != NULL);
  const int           has_comp_density = (composition_density != NULL);
  ...
  if (has_comp_density) {
    RHEA_ASSERT (rhea_composition_check_vec_type (composition_density));
    RHEA_ASSERT (d->composition_density_original == NULL);

    d->composition_density_original = rhea_stokes_problem_amr_field_to_buffer (
        composition_density);
    RHEA_ASSERT (d->composition_density_original->n == d->ymir_mesh->ma->Np);
  }
...
rhea_stokes_problem_amr_data_finalize_fn (p4est_t *p4est, void *data)
{
  ...
  const int           has_comp_density = (d->composition_density_original != NULL);
  ymir_vec_t         *temperature, *velocity_pressure;
  ymir_vec_t         *composition_density;
  ...
  if (has_comp_density) {
    composition_density = rhea_composition_new (d->ymir_mesh);
    rhea_stokes_problem_amr_buffer_to_field (
        composition_density, d->composition_density_original, d->press_elem);
  }
  ...
  if (has_comp_density) {
    rhea_stokes_problem_set_composition (d->stokes_problem, composition_density,
                                         NULL /* comp. viscosity */);
  }
...
rhea_stokes_problem_amr_data_project_fn (p4est_t *p4est, void *data)
{
  ...
  const int           has_vel = (d->velocity_original != NULL);
  const int           has_press = (d->pressure_original != NULL);
  const int           has_comp_density =
                        (d->composition_density_original != NULL);
  ...
  if (has_comp_density) {
    RHEA_ASSERT (d->composition_density_adapted == NULL);
    d->composition_density_adapted = rhea_stokes_problem_amr_project_field (
        d->composition_density_original, 1 /* #fields */,
        d->mangll_original, d->mangll_adapted);
    d->composition_density_original = NULL;
  }
...
rhea_stokes_problem_amr_data_partition_fn (p4est_t *p4est, void *data)
{
  ...
  const int           has_comp_density =
                        (d->composition_density_adapted != NULL);
  ...
  if (has_comp_density) {
    RHEA_ASSERT (d->composition_density_original == NULL);
    d->composition_density_original = rhea_stokes_problem_amr_partition_field (
        d->composition_density_adapted, 1 /* #fields*/,
        d->mangll_adapted, d->mangll_partitioned);
    d->composition_density_adapted = NULL;
  }
