                                ...
                                rhea_domain_options_t *opt)
{
  ...
  /* compute radius */
//  radius = rhea_domain_compute_radius (*x, *y, *z, opt); //XI

  /* map coordinates onto surface parallel to radius */
  switch (opt->shape) {
  case RHEA_DOMAIN_CUBE:
  case RHEA_DOMAIN_BOX:
//    *z *= radius_max / radius;  //XI
    *z = opt->z_max; //XI TODO double check with Johann. what is the point of computing radius for BOX? Also, only after this modification can I get right answer using rhea_topography_displacement_node
    break;
  case RHEA_DOMAIN_SHELL:
  case RHEA_DOMAIN_CUBE_SPHERICAL:
  case RHEA_DOMAIN_BOX_SPHERICAL:
