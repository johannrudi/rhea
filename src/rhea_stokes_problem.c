...
//###DEV### testing another dual variable update
#define RHEA_STOKES_PROBLEM_NONLINEAR_NEWTON_DUAL_VARIABLE 0
...
struct rhea_stokes_problem
{
  ...
  int                                    rhs_vel_compute_fn_data_owned;
  ...
  ymir_vec_t         *marker;           /* out: linear & nonlinear Stokes */
                                ...
                                rhea_viscosity_options_t *visc_options)
{
  ...
  stokes_problem->marker = NULL;
  stokes_problem->weakzone = NULL;
  //stokes_problem->composition_viscosity = NULL;
  stokes_problem->sol_vel = NULL;
  stokes_problem->proj_tens = NULL;

  stokes_problem->composition_density = NULL;
  ...
  /* set default callback for viscosity */
  ...
  /* set default callback for right-hand side */
  {
    rhea_velocity_rhs_compute_data_t *rhs_data;

    rhs_data = RHEA_ALLOC (rhea_velocity_rhs_compute_data_t, 1);
    rhs_data->temp_options = temp_options;
    rhs_data->comp_options = comp_options;

    stokes_problem->rhs_vel_compute_fn = rhea_velocity_rhs_compute;
    stokes_problem->rhs_vel_compute_fn_data = rhs_data;
    stokes_problem->rhs_vel_compute_fn_data_owned = 1;
  }
...
{
  /* destroy right-hand side data */
  if (stokes_problem->rhs_vel_compute_fn_data_owned) {
    rhea_velocity_rhs_compute_data_t *d =
...
/**
 * Retrieves velocity (static use only because a pointer to data inside
 * `rhea_stokes_problem_t` is returned).
 */
static ymir_vec_t *
                                         ...
                                         const int nonlinear_init)
{
  ...
  ymir_vec_t         *marker      = stokes_problem->marker;
                                       ...
                                       double residual_reduction[3])
{
  ...
  int                 stop_reason, itn;
  double              norm_res_init = NAN, norm_res = NAN, norm_rhs = NAN;
  double              norm_res_init_vel = NAN, norm_res_vel = NAN;
  double              norm_res_init_press = NAN, norm_res_press = NAN;
  double              res_reduction, res_reduction_vs_rhs;
  double              res_reduction_vel, res_reduction_press;

  RHEA_GLOBAL_VERBOSEF_FN_BEGIN (
      __func__, "linearized=%i, incompressible=%i, krylov_idx=%i, "
      "iter_max=%i, rtol=%.1e, nonzero_init_guess=%i",
      linearized, incompressible, krylov_solver_idx,
      iter_max, rtol, nonzero_initial_guess);

  /* check input */
  RHEA_ASSERT (stokes_op != NULL);
  RHEA_ASSERT (stokes_pc != NULL);
  ...
  if (check_residual) {
    ...
    norm_res_init = norm_rhs = rhea_stokes_norm_compute (
        &norm_res_init_vel, &norm_res_init_press, rhs_vel_press,
        RHEA_STOKES_NORM_L2_VEC_SP, NULL, press_elem);
  ...
  stop_reason = ymir_stokes_pc_solve (
      rhs_vel_press, sol_vel_press, stokes_pc, nonzero_initial_guess,
      rtol, atol, iter_max, krylov_solver_idx, &itn);
  ...
  if (check_residual) {
    ...
    res_reduction_vs_rhs =
      rhea_stokes_problem_calculate_reduction (norm_rhs, norm_res);
    res_reduction_vel =
    ...
    RHEA_GLOBAL_VERBOSEF_FN_TAG (
        ...
        "residual_reduction=%.3e, residual_reduction_vs_rhs=%.3e",
        stop_reason, itn, res_reduction, res_reduction_vs_rhs);
                                    ...
                                    rhea_stokes_problem_t *stokes_problem_lin)
{
  ...
  vectors_exist = (
      ...
      stokes_problem_lin->marker != NULL &&
                                    ...
                                    ymir_pressure_elem_t *press_elem)
{
  ...
  stokes_problem_lin->marker = rhea_viscosity_new (ymir_mesh);
  ...
  stokes_problem_lin->rhs_vel_compute_fn (
      ...
      stokes_problem_lin->composition_density,
                                    ...
                                    rhea_stokes_problem_t *stokes_problem_lin)
{
  ...
  rhea_viscosity_destroy (stokes_problem_lin->marker);
                                ...
                                rhea_viscosity_options_t *visc_options)
{
  ...
  stokes_problem_lin = rhea_stokes_problem_struct_new (
      RHEA_STOKES_PROBLEM_LINEAR, domain_options, temp_options, comp_options,
  ...
  /* set composition */
  rhea_stokes_problem_set_composition (stokes_problem_lin,
                                    ...
                                    rhea_stokes_problem_t *stokes_problem_lin)
{
  ...
  stokes_problem_lin->stokes_pc = ymir_stokes_pc_new_multi_krylov (
                                  ...
                                  double *residual_reduction)
{
  ...
  stop_reason = rhea_stokes_problem_run_krylov_solver (
      *sol_vel_press, rhs_vel_press, nonzero_initial_guess, iter_max, rtol,
      ...
      stokes_problem_lin->stokes_op, 0 /* !linearized */,
      stokes_problem_lin->incompressible, krylov_solver_idx,
      stokes_problem_lin->press_elem,
                                                 ...
                                                 void *data)
{
  ...
  case RHEA_STOKES_PROBLEM_NONLINEAR_LINEARIZATION_NEWTON_REGULAR:
    {
      ...
      if (solution_exists) { /* if solution is provided */
        ...
        ymir_stress_op_optimized_compute_strain_rate (
            proj_tens, sol_vel, stress_op, 1 /* nonzero Dir */);
      ...
      if (solution_exists) { /* if solution is provided */
        ...
        if (step_exists) { /* if step is provided */
          ...
          ymir_stress_op_optimized_compute_visc_stress (
              prev_stress, prev_vel, stress_op,
              0 /* !linearized */, 1 /* nonzero Dir */);
         ...
         * Newton */
        {
          ...
          ymir_stress_op_optimized_compute_strain_rate (
              proj_tens_strain, sol_vel, stress_op, 1 /* nonzero Dir */);
  ...
  case RHEA_STOKES_PROBLEM_NONLINEAR_LINEARIZATION_NEWTON_DEV1:
    {
      ...
      RHEA_ASSERT (stokes_problem_nl->proj_scal != NULL);
      RHEA_ASSERT (rhea_viscosity_check_vec_type (proj_scal));
      RHEA_ASSERT (sc_dmatrix_is_valid (proj_scal->dataown));
      ...
      if (solution_exists) { /* if solution is provided */
        ...
        if (step_exists) { /* if step is provided */
          ...
          ymir_stress_op_optimized_compute_strain_rate (
              prev_strain, prev_vel, stress_op, 1 /* nonzero Dir */);
          ...
          ymir_stress_op_optimized_compute_visc_stress_override_viscosity (
              proj_tens_model, step_vel, stress_op,
              1 /* linearized */, 0 /* zero Dir */, visc_invisible);
          rhea_viscosity_destroy (visc_invisible);
          RHEA_ASSERT (rhea_stress_is_valid (proj_tens_model));

#if (0 < RHEA_STOKES_PROBLEM_NONLINEAR_NEWTON_DUAL_VARIABLE)
          /* combine prev strain rate and step */
          ymir_vec_add (1.0, prev_strain, proj_tens_model);
          ymir_vec_scale (step_length, proj_tens_model);
          RHEA_ASSERT (rhea_stress_is_valid (proj_tens_model));

          /* divide by 2nd invariant of the (prev) strain rate */
          {
            ymir_vec_t         *prev_strain_2inv =
                                  rhea_strainrate_2inv_new (ymir_mesh);
            double              prev_strain_2inv_reg;

            rhea_strainrate_compute_sqrt_of_2inv (prev_strain_2inv, prev_vel);
            ymir_vec_scale (M_SQRT2, prev_strain_2inv);
            prev_strain_2inv_reg = 1.0e-3*ymir_vec_max_global (prev_strain_2inv);
            if (0.0 < prev_strain_2inv_reg) {
              ymir_vec_bound_min (prev_strain_2inv, prev_strain_2inv_reg);
              ymir_vec_divide_in1 (prev_strain_2inv, proj_tens_model);
              RHEA_ASSERT (rhea_stress_is_valid (proj_tens_model));
            }

            rhea_strainrate_2inv_destroy (prev_strain_2inv);
          }
#else
          /* combine prev strain rate and step */
          ymir_vec_scale (step_length, proj_tens_model);
          ...
          ymir_stress_op_optimized_compute_strain_rate (
              proj_tens_model, sol_vel, stress_op, 1 /* nonzero Dir */);
          RHEA_ASSERT (rhea_stress_is_valid (proj_tens_model));
        }

#if (0 < RHEA_STOKES_PROBLEM_NONLINEAR_NEWTON_DUAL_VARIABLE)
        /* add previous tensor */
        if (isfinite (step_length)) {
          ymir_vec_add (1.0 - step_length, stokes_problem_nl->proj_tens,
                        proj_tens_model);
          RHEA_ASSERT (rhea_stress_is_valid (proj_tens_model));
        }

        /* compute pointwise norm of tensor and transfer to proj. scaling
         * (this is necessary because the proj. tensor will be normalized) */
        {
          ymir_vec_t         *ptw_norm = rhea_strainrate_2inv_new (ymir_mesh);

          ymir_stress_op_tensor_get_pointwise_norm (ptw_norm, proj_tens_model);
          ymir_vec_bound_max (ptw_norm, 1.0);
          RHEA_ASSERT (0.0 <= ymir_dvec_min_local (ptw_norm));
          ymir_vec_multiply_in (ptw_norm, proj_scal);
          RHEA_ASSERT (sc_dmatrix_is_valid (proj_scal->dataown));

          rhea_strainrate_2inv_destroy (ptw_norm);
        }

        /* store copy of tensor */
        ymir_vec_copy (proj_tens_model, stokes_problem_nl->proj_tens);
#endif
  ...
  if (stokes_problem_nl->stokes_pc == NULL) {
    ...
    stokes_problem_nl->stokes_pc = ymir_stokes_pc_new_multi_krylov (
                                            ...
                                            ymir_vec_t *solution, void *data)
{
  ...
  if (solution != NULL) {
    ...
    ymir_stokes_pc_apply_stokes_op (solution, neg_gradient, stokes_op,
                                                      ...
                                                      double *norm_comp)
{
  ...
  /* avoid negative values due to rounding and communication */
  ip_vel   = SC_MAX (0.0, ip_vel);
  ip_press = SC_MAX (0.0, ip_press);
                                                ...
                                                void *data)
{
  ...
  /* apply */
  ymir_stokes_pc_apply_stokes_op (in, out, stokes_op, 1 /* linearized */,
                                  1 /* dirty */);
                                            ...
                                            int *lin_iter_count)
{
  ...
  stop_reason = rhea_stokes_problem_run_krylov_solver (
      ...
      stokes_problem_nl->incompressible, krylov_solver_idx,
      stokes_problem_nl->press_elem,
      lin_iter_count, NULL /* res reduc */);
                                                 ...
                                                 const int iter, void *data)
{
  ...
  if (NULL != stokes_problem_nl->plate_options &&
    ...
    case RHEA_DOMAIN_SHELL:
      pid_begin = RHEA_PLATE_EARTH_MORVEL25_BEGIN;
      pid_end   = RHEA_PLATE_EARTH_MORVEL25_END;
      break;
    default:
      pid_begin = 0;
      pid_end   = n_plates - 1;
    ...
    /* print */
    for (pid = pid_begin; pid <= pid_end; pid++) {
      RHEA_GLOBAL_STATISTICSF (
                                    ...
                                    ymir_pressure_elem_t *press_elem)
{
  ...
  stokes_problem_nl->marker = rhea_viscosity_new (ymir_mesh);
  ...
  stokes_problem_nl->rhs_vel_compute_fn (
      ...
      stokes_problem_nl->composition_density,
...
#if (0 < RHEA_STOKES_PROBLEM_NONLINEAR_NEWTON_DUAL_VARIABLE)
    stokes_problem_nl->proj_tens = rhea_strainrate_new (ymir_mesh);
    ymir_vec_set_zero (stokes_problem_nl->proj_tens);
#endif
    break;
  default: /* unknown linearization type */
    RHEA_ABORT_NOT_REACHED ();
                                    ...
                                    rhea_stokes_problem_t *stokes_problem_nl)
{
  ...
  case RHEA_STOKES_PROBLEM_NONLINEAR_LINEARIZATION_NEWTON_DEV1:
    ...
    rhea_viscosity_destroy (stokes_problem_nl->proj_scal);
#if (0 < RHEA_STOKES_PROBLEM_NONLINEAR_NEWTON_DUAL_VARIABLE)
    rhea_viscosity_destroy (stokes_problem_nl->proj_tens);
#endif
    break;
  default: /* unknown linearization type */
    RHEA_ABORT_NOT_REACHED ();
  ...
  rhea_viscosity_destroy (stokes_problem_nl->marker);
                                   ...
                                   rhea_viscosity_options_t *visc_options)
{
  ...
  stokes_problem_nl = rhea_stokes_problem_struct_new (
      RHEA_STOKES_PROBLEM_NONLINEAR, domain_options, temp_options,
      comp_options, plate_options, weak_options, visc_options);

  /* set temperature */
  rhea_stokes_problem_set_temperature (stokes_problem_nl, temperature);

  /* set composition */
  rhea_stokes_problem_set_composition (stokes_problem_nl,
                                       composition /* comp. density */,
                                       NULL /* comp. viscosity */);
                         ...
                         rhea_viscosity_options_t *visc_options)
{
  ...
  case RHEA_VISCOSITY_LINEAR:
    return rhea_stokes_problem_linear_new (
        ...
        temp_options, comp_options, NULL /* plate_options */,
    ...
    if (force_linear_solve) { /* if run only a single linear solve */
      ...
      stop_reason = rhea_stokes_problem_run_krylov_solver (
          *sol_vel_press, stokes_problem->rhs_vel_press,
          ...
          stokes_problem->stokes_op, 1 /* linearized */,
          stokes_problem->incompressible, krylov_solver_idx,
          stokes_problem->press_elem,
...
rhea_stokes_problem_set_composition (rhea_stokes_problem_t *stokes_problem,
                                     ...
                                     ymir_vec_t *composition_viscosity)
{
  stokes_problem->composition_density = composition_density;
  //stokes_problem->composition_viscosity = composition_viscosity;
}

ymir_vec_t *
rhea_stokes_problem_get_composition_density (rhea_stokes_problem_t *stokes_problem)
{
  return stokes_problem->composition_density;
}

//ymir_vec_t *
//rhea_stokes_problem_get_composition_viscosity (rhea_stokes_problem_t *stokes_problem)
//{
//  return stokes_problem->composition_viscosity;
//}
                                    ...
                                    void *data)
{
  ...
  stokes_problem->rhs_vel_compute_fn_data_owned = 0;
  ...
  stokes_problem->rhs_vel_compute_fn (
      ...
      stokes_problem->composition_density,
      stokes_problem->rhs_vel_compute_fn_data);
}
                                    ...
                                    const int skip_pressure)
{
  ...
  /* reset linearized flag for linear Stokes problems */
  if (stokes_problem->type == RHEA_STOKES_PROBLEM_LINEAR) {
    linearized_visc_coeff = 0;
  }

  /* get viscous stress operator */
  if (NULL != override_stress_op) {
    stress_op = override_stress_op;
  }
  else {
    data_exists_lin = (
                                    ...
                                    int linearized_visc_coeff)
{
  ...
  /* check input */
  RHEA_ASSERT (rhea_velocity_check_vec_type (velocity));
  RHEA_ASSERT (rhea_stress_check_vec_type (stress_like_tensor) ||
               rhea_stress_nonsymmetric_check_vec_type (stress_like_tensor));
  RHEA_ASSERT (rhea_stress_is_valid (stress_like_tensor));
  ...
  ymir_stress_op_optimized_compute_stress_div (
      velocity, stress_like_tensor, stress_op, linearized_visc_coeff,
      0 /* zero Dir */);
