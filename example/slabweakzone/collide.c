...
typedef enum
{
  COLLIDE_TEST_TI_MANUFACTURED_NONE = 0,
  COLLIDE_TEST_TI_MANUFACTURED_SINCOS_ISO,
  COLLIDE_TEST_TI_MANUFACTURED_SINCOS_ANISO
}
collide_test_TI_manufactured_t;
...
typedef struct collide_options
{
  ...
  collide_test_TI_manufactured_t  test_TI_manufactured;
}
collide_options_t;
...
static void         collide_test_TI_stress_op (
                                        ...
                                        const char *vtk_path);
static void         collide_test_TI_manufactured (
                                        collide_options_t *collide_options,
                                        ymir_stress_op_t *stress_op,
                                        ymir_vec_t *vel_in,
                                        ymir_vec_t *pres_in,
                                        ymir_vec_t *rhs_manufactured);

static void         collide_test_compute_error_vel (
...
main (int argc, char **argv)
{
  ...
  YMIR_OPTIONS_I, "test-TI-manufactured", '\0',
    &(test_TI_manufactured), COLLIDE_TEST_TI_MANUFACTURED_NONE,
    "Runs test of anisotropic viscosity using manufactured solution approach",
  ...
  YMIR_OPTIONS_S, "vtk-write-test-path", '\0',
    ...
    "File path for vtk files for test TI stress operator results",
  ...
  collide_options.test_TI_stress_op =
    (collide_test_TI_stress_op_t) test_TI_stress_op;
  collide_options.test_TI_manufactured =
    (collide_test_TI_manufactured_t) test_TI_manufactured;
  ...
  if (collide_options.test_TI_manufactured != COLLIDE_TEST_TI_MANUFACTURED_NONE) {
    ymir_vec_t         *vel_in, *pres_in, *rhs_manufactured;
    ymir_vec_t         *vel_chk, *pres_chk, *vel_err, *pres_err;
    double             vel_abs_error, vel_rel_error;
    double             pres_abs_error, pres_rel_error;
    ymir_stokes_op_t   *stokes_op;
    ymir_stress_op_t   *stress_op;
    char                vtk_path[BUFSIZ];
    snprintf (vtk_path, BUFSIZ, "%s_manufactured", vtk_write_test_path);
    ...
    vel_err = rhea_velocity_new (ymir_mesh);
    pres_in = rhea_velocity_new (ymir_mesh);
    pres_chk = rhea_velocity_new (ymir_mesh);
    pres_err = rhea_velocity_new (ymir_mesh);
    rhs_manufactured = rhea_velocity_new (ymir_mesh);

    collide_test_TI_manufactured (&collide_options, stress_op,
                                  vel_in, pres_in, rhs_manufactured);
    stokes_problem->rhs_vel_press = rhs_manufactured;
    ...
    pres_err = rhea_velocity_new (ymir_mesh);
    /* calculate error */
    collide_test_compute_error_vel (&vel_abs_error, &vel_rel_error, vel_err,
                                    vel_in, vel_chk, stress_op);
    collide_test_compute_error_vel (&pres_abs_error, &pres_rel_error, pres_err,
                                    pres_in, pres_chk, stress_op);
    rhea_global_infof ("%s (test type %i): velocity: abs error %1.3e rel error %1.3e\n; pressure: abs error %1.3e rel error %1.3e\n",
                       this_fn_name, test_TI_manufactured, vel_abs_error, vel_rel_error,
                       pres_abs_error, pres_rel_error);
    ...
    rhea_velocity_destroy (pres_in);
    rhea_velocity_destroy (pres_chk);
    rhea_velocity_destroy (pres_err);
    rhea_velocity_destroy (rhs_manufactured);
...
static void
collide_test_TI_manufactured (collide_options_t *collide_options,
                              ymir_stress_op_t *stress_op,
                              ymir_vec_t *vel_in,
                              ymir_vec_t *pres_in,
                              ymir_vec_t *rhs_manufactured)
{
  const char         *this_fn_name = "collide_test_TI_manufactured";
  const collide_test_TI_manufactured_t
                      test_type = collide_options->test_TI_manufactured;
  ymir_vec_t         *coeff_TI_svisc, *coeff_TI_tensor;

  /* check input */
  RHEA_ASSERT (test_type != COLLIDE_TEST_TI_MANUFACTURED_NONE);
  ...
  ymir_vec_sec_zero (pres_in);
  /* compute velocity fields */
  switch (test_type) {
    case COLLIDE_TEST_TI_MANUFACTURED_SINCOS_ISO:
      /* compute reference velocity field (output) */
      ymir_cvec_set_function (vel_in, collide_test_TI_sincos_iso_vel_out_fn,
                              NULL);
      ymir_mass_apply (vel_in, rhs_manufactured);
    ...
    case COLLIDE_TEST_TI_MANUFACTURED_SINCOS_ANISO:
      /* compute reference velocity field (output) */
      ymir_cvec_set_function (vel_in, collide_test_TI_sincos_aniso_vel_out_fn,
                              NULL);
      ymir_mass_apply (vel_in, rhs_manufactured);
